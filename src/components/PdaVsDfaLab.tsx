...
export function PdaVsDfaLab({ onOpenDebugger }: PdaVsDfaLabProps) {
  ...
  return (
    <section className="panel dfa-pda-lab">
      <div className="panel-heading"><div><span>PDA vs DFA</span></div><span>aⁿbⁿ</span></div>
      ...
      <div className="memory-compare-grid">
        <article className={`memory-card finite-memory ${overflow ? 'overflow' : ''}`}>
          <header><strong>Finite control only</strong></header>
          ...
          <div className="memory-verdict"><span>{overflow ? 'Over capacity' : 'Within capacity'}</span><small>Demo only — not a proof.</small></div>
        </article>

        <article className="memory-card stack-memory">
          <header><strong>Unbounded stack memory</strong></header>
          ...
          <div className="memory-verdict accepted"><span>{accepting ? 'Accepted' : tree.truncated ? 'Search limit' : 'Rejected'}</span><small>{accepting ? `Accepts at depth ${accepting.depth}.` : 'No accepting path found.'}</small></div>
        </article>
      </div>
      ...
      <div className="formal-note">
        ...
        <button onClick={() => onOpenDebugger(input)}>Open in debugger</button>
      </div>
    </section>
  )
}
