  ...
  return (
    <section className="testbench-workspace">
      <div className="testbench-toolbar">
        ...
        <div className="testbench-title"><small>LANGUAGE TEST BENCH</small><strong>Assertions against the current PDA</strong></div>
        <div className={`testbench-summary ${passed === results.length && !parsed.errors.length ? 'pass' : 'mixed'}`}>
          <strong>{passed} / {results.length}</strong><small>ASSERTIONS PASSING</small>
        </div>
      </div>

      <div className="testbench-grid">
        <section className="panel assertion-editor-panel">
          <div className="panel-heading"><div><span>ASSERTIONS</span><span className="heading-separator">/</span><span>EDITOR</span></div><span>{mode === 'final-state' ? 'FINAL STATE' : 'EMPTY STACK'}</span></div>
        ...
        <section className="panel test-results-panel">
          <div className="panel-heading"><div><span>RESULTS</span><span className="heading-separator">/</span><span>{results.length} TESTS</span></div><span>{limited ? `${limited} LIMITED` : 'BOUNDED SEARCH COMPLETE'}</span></div>
          <div className="test-results-head" aria-hidden="true"><span>TEST</span><span>INPUT</span><span>EXPECTED</span><span>ACTUAL</span><span>CONFIGS</span><span>ACTION</span></div>
          <div className="test-results-list">
            {results.map((result, index) => (
              <article key={result.id} className={`test-result ${result.passed ? 'pass' : result.outcome === 'limit' ? 'limit' : 'fail'}`}>
                ...
                <span className={`outcome ${result.outcome}`}>{result.outcome.toUpperCase()}</span>
