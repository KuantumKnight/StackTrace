...
export function ExperienceLayer() {
  ...
  useEffect(() => {
    const revealTargets = Array.from(document.querySelectorAll<HTMLElement>('.workspace-hero, .learning-thread, .workspace-stage, .playback-dock, .inspector-shell, .panel'))
  ...
  const cycleTheme = () => setTheme((current) => current === 'paper' ? 'glass' : current === 'glass' ? 'night' : 'paper')
    ...
    <>
      ...
      <div className="experience-controls" aria-label="Experience controls">
        <button className="icon-button theme-button" type="button" onClick={cycleTheme} aria-label={`Theme: ${theme}. Change theme`} title={`Theme: ${theme}`}>
          <span aria-hidden="true">{theme === 'night' ? '☾' : theme === 'glass' ? '◐' : '☼'}</span>
        </button>
      ...
      {showTop && <button className="back-to-top" type="button" aria-label="Back to top" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>↑</button>}
          ...
          <section className="command-palette" role="dialog" aria-modal="true" aria-label="Command palette">
            ...
            <div className="command-list">
              {filteredCommands.map((command) => (
                <button key={command.id} type="button" onClick={() => { command.action(); setPaletteOpen(false); setQuery('') }}>
